...
*Integration* is the act of embedding the `BlueSteel.framework` binary into your project, thereby exposing the API it provides to your code.

BlueSteel is built as a *Swift framework*, and as such, it has the following base platform requirements:
...
BlueSteel is **Swift 2.1**-compliant and requires **Xcode 7.1 or higher** to be built.
...
The steps below have been tested with **git 2.4.9 (Apple Git-60)**, although they should be compatible with a wide range of git versions.
...
- **[Carthage integration](#carthage-integration)** — Explains how to use the [Carthage](https://github.com/Carthage/Carthage) dependency manager for adding BlueSteel to your project.

- **[Manual integration](#manual-integration)** — Demonstrates the steps for adding BlueSteel by embedding `BlueSteel.xcodeproj` within your own Xcode project. (**Note:** Manual integration is a bit more involved than using Carthage).
...
> As of this writing, the current version of Carthage is 0.10.
...
When building a framework for a platform with a simulator, Carthage creates a *universal binary*, allowing `BlueSteel.framework` to work in the simulator as well as on actual devices.
...
1. Update the `Cartfile` with an entry for BlueSteel
2. Download and build BlueSteel
3. Add `BlueSteel.framework` to your application target
...
The command `carthage update` causes Carthage to download and build BlueSteel.

By default, `carthage update` builds all platform targets in a project. Normally, this is _not_ what you want; you’ll usually use BlueSteel on just one platform at a time.
...
Once Carthage is done building BlueSteel, you can use the `open` command to locate the framework binaries in Finder:
...
If all went well, the Carthage build directories should contain the file `BlueSteel.framework`. If that file isn’t present, something went wrong with the build.
...
Go back to Finder, select the file `BlueSteel.framework`, and then drag it into the list area directly below *Embedded Binaries*.

If successful, you should see `BlueSteel.framework` listed under both the *Embedded Binaries* and *Linked Frameworks and Libraries* sections.
...
```
$PROJECT_DIR/Carthage/Checkouts/BlueSteel/BuildControl/bin/stripCarthageFrameworks.sh
```
...
Once you’ve done this, try building your application. If you don’t see any errors, **_you’re all done integrating BlueSteel!_**

But before you start coding, skip to the [Adding the Swift import](#adding-the-swift-import) section to see how you can import BlueSteel for use in your Swift code.

## Manual Integration

Manual integration involves embedding `BlueSteel.xcodeproj` directly in your Xcode project.

This will ensure that BlueSteel is built with the exact same settings you’re using for your app. You won’t have to fiddle with different settings for different architectures — when you’re running in the simulator, it will work; when you then switch to building for device, it’ll work, too.

You’ll also be able to step into BlueSteel code directly in the debugger without worrying about `.dSYM` resolution, which is very helpful.

### An Overview of the Process

Manual integration is a bit involved; there are five high-level tasks that you’ll need to perform:

1. Download the BlueSteel source into your project structure
2. Embed `BlueSteel.xcodeproj` in your Xcode project
3. Build `BlueSteel.framework`
4. Add `BlueSteel.framework` to your application target
...
For our integration examples, we’re going to be showing the top-level `BlueSteel` directory inside a `Libraries` directory at the root level of of your application’s source.
...
### 1. Download the BlueSteel source

If you’re already using git for version control, we recommend adding BlueSteel to your project as a submodule. This will allow you to “lock” your codebase to specific versions of BlueSteel, making it easier to incorporate new versions on whatever schedule works best for you.

If you’re using some other form of version control of if you’re not using version control at all—*shame on you!*—then you’ll want to *clone* the BlueSteel repository. We suggest putting the BlueSteel clone somewhere within your application’s directory structure, so that it is included in whatever version control regimen you’re using.

#### Downloading BlueSteel as a submodule

> **Important:** Skip this section if you plan to download BlueSteel using `git clone`.

From within the `Libraries` directory, issue the following commands to download BlueSteel:

```bash
git submodule add https://github.com/gilt/BlueSteel
git submodule update --init --recursive
```

Next, you’re ready to [embed the `BlueSteel.xcodeproj` project file in your Xcode project](#2-embed-bluesteel-in-your-project).

#### Downloading BlueSteel as a cloned repo

> **Important:** Skip this section if you already performed the tasks outlined in “Downloading BlueSteel as a submodule” above.

From within the `Libraries` directory, issue the following command to clone the BlueSteel repository:

```bash
git clone https://github.com/gilt/BlueSteel
```

### 2. Embed BlueSteel in your project

In the Terminal, the command `open BlueSteel` to open the folder containing the BlueSteel source in the Finder. This will reveal the `BlueSteel.xcodeproj` Xcode project and all files needed to build `BlueSteel.framework`.

Then, open your application in Xcode, and drag `BlueSteel.xcodeproj` into the Xcode project browser. This will embed BlueSteel in your project and allow you to add the targets built by BlueSteel to your project.

### 3. Build BlueSteel

Before we can add `BlueSteel.framework` to your app, we have to build it, so Xcode has more information about the framework.
...
Once a device-based run destination has been selected, select the appropriate build scheme for the target platform: “BlueSteel-iOS”, “BlueSteel-OSX”, “BlueSteel-tvOS” or “BlueSteel-watchOS”.

Then, select *Build* (⌘B) from the *Product* menu.

Once the build is complete, open `BlueSteel.xcodeproj` in the project navigator and find the “Products” group. Open that, and right-click on `BlueSteel.framework`. Select *Show in Finder*. This will open the folder containing the framework binary you just built.
...
So, to make things sane again, you’ll need to make sure Xcode references `BlueSteel.framework` using a “Relative to Build Products” location.

To do this:

1. Locate `BlueSteel.framework` in the Xcode project browser
...
Once you’ve done this for each framework, **_you’re all done integrating BlueSteel!_**

## Adding the Swift import

Once BlueSteel has been successfully integrated, all you will need to do is add the following `import` statement to any Swift source file where you want to use BlueSteel:

```swift
import BlueSteel
```

Want to learn more about BlueSteel? Check out [the README](https://github.com/gilt/BlueSteel/blob/master/README.md).
